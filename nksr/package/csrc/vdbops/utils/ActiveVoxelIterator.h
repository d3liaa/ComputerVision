 ...
 * We need this because nanovdb::NanoTree<nanovdb::ValueIndex> has a data type mapping to
 * nanovdb::ValueIndex (which is a dummy struct), but the actual data is uint64_t.
 *
...
template <>
struct ActiveVoxelIteratorDataTypeExtractor<nanovdb::NanoTree<nanovdb::ValueIndex>> {
 ...
 */
template <typename TreeT, int64_t Offset=0>
struct ActiveVoxelIterator {
    ...
    ActiveVoxelIterator(const TreeT& tree) {
