...
struct BezierInterpolationWithGradIterator {
    ...
    __hostdev__ BezierInterpolationWithGradIterator(const nanovdb::Vec3<ScalarT>& p) {
        mCount = 0;
        mVoxel = p.round();
        mUVW = p - nanovdb::Vec3<ScalarT>(mVoxel.x(), mVoxel.y(), mVoxel.z());
...
protected:
    ...
    __hostdev__ inline void updateCoordAndWeight() {
        ...
        mCoordAndWXYZ = {nanovdb::Coord(dx, dy, dz) + mVoxel, nanovdb::Vec4<ScalarT>(
